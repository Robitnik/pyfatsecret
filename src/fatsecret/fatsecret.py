...
class Fatsecret:
    ...
    def __init__(self, consumer_key, consumer_secret, session_token=None,
        ...
        self.request_token = self.request_token_secret = None
        self.access_token  = self.access_token_secret  = None

        if session_token:
            self.access_token, self.access_token_secret = session_token
            self.session = self._make_session(self.access_token,
    ...
    def _make_session(self,
        ...
        return OAuth1Session(
            ...
            callback_uri=self.callback_url if with_callback else None,
            verifier=verifier,
    ...
    def get_authorize_url(self) -> str:
        ...
        token = oauth_tmp.fetch_request_token(
        ...
        return oauth_tmp.authorization_url(self.AUTHORIZE_URL)


    def authenticate(self, oauth_verifier: str):
        """Retrieve access tokens once user has approved access to authenticate session

        :param oauth_verifier: PIN displayed to user or returned by authorize_url when callback url is provided
        :type oauth_verifier: int
        """

        oauth = self._make_session(
            resource_owner_key=self.request_token,
            resource_owner_secret=self.request_token_secret,
            verifier=oauth_verifier,
        )
        acc = oauth.fetch_access_token(self.ACCESS_TOKEN_URL)
        self.access_token        = acc["oauth_token"]
        self.access_token_secret = acc["oauth_token_secret"]
        self.session = self._make_session(self.access_token,
                                          self.access_token_secret)
    ...
    def valid_response(response: requests.Response):
        ...
        for key in data:
            ...
            if key == "error":
                ...
                if code in (2, *range(3, 10)):
                    raise AuthenticationError(code, msg)
                elif code in (1, 10, 11, 12, 20, 21):
                    raise GeneralError(code, msg)
                elif 101 <= code <= 108:
                    raise ParameterError(code, msg)
                elif 201 <= code <= 207:
                    raise ApplicationError(code, msg)
            # ---- успіх / корисні дані ----
            elif key == "success":
                return True
            elif key in (
                "food", "recipe", "recipe_types",
                "saved_meal_id", "saved_meal_item_id",
                "food_entry_id",
            ):
                return data[key]
            elif key == "foods":
                return data[key]["food"]
            elif key == "suggestions":
                return data[key]
            elif key == "recipes":
                return data[key]["recipe"]
            elif key == "saved_meals":
                return data[key]["saved_meal"]
            elif key == "saved_meal_items":
                return data[key]["saved_meal_item"]
            elif key == "exercise_types":
                return data[key]["exercise"]
            elif key == "food_entries":
                entries = data[key]
                if entries is None:
                    return []
                entries = entries["food_entry"]
                ...
                prof = data[key]
                return (
                    (prof["auth_token"], prof["auth_secret"])
                    if "auth_token" in prof
                    else prof
                )
        return data

    def food_get(self, food_id: str):
        params = {"method": "food.get", "food_id": food_id, "format": "json"}
        resp = self.session.get(self.api_url, params=params)
        resp.raise_for_status()
        return self.valid_response(resp)

    def food_get_v2(self, food_id: str, region: str = None, language: str = None):
    ...
    def food_find_id_for_barcode(
        self, barcode: str, region: str = None, language: str = None
    ):
        params = {
    ...
    def foods_get_most_eaten(self, meal: str = None):
    ...
    def foods_get_recently_eaten(self, meal: str = None):
